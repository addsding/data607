...
Almost every show has the concept of a "crown" (\* = triple crown, \*\* = quintuple crown) which denotes that once a song has won three or five times, it is ineligible from winning again.
...
-   A Google sheet of scraped music show wins (all sourced from Wikipedia)
...
-   Crown: Whether this song achieved a triple/quintuple crown
...
Next, we'll be taking subsets of the Wikipedia data to prep for joining. As we have one table (`all_groups`) holding group information and then one table (`idols`) for soloists, we'll have to eventually do two joins.
...
Note that `wins` had rows for days where there weren't shows being broadcasted or no winning song; by doing these joins in this section, we're removing these rows. This means that the ending dataframe will likely have less rows than the one we began with (`wins`).

#### Group Wins

With these two subsets created now, we'll start with `group_wins` -- this is joined to `all_groups` based on `artist_name`. For simplicity, let's change the case for all strings to lower in order to ensure that the join will work. This will be a left join with `group_wins` as the base and `all_groups` appending all its data.
...
As there are 22 missing artists here, I'll opt to add these directly to the idols .csv and reload that file. I'll also only be adding relevant data to avoid researching too many irrelevant things (birth place, birthday, etc.) before rejoining it to `solo_wins`.
...
With this chart, we can kind of see the rise of some groups and the fall of others; EXO and GFRIEND had many more wins in 2016, but that number slowly dwindles as years pass. On the opposite side, BTS spikes in popularity in 2020 after a gradual increase past 20 wins before skyrocketing almost to 50. It's also interesting how we see Twice and Wanna One appear in 2017 with the latter disappearing after 2019 (this is due to Wanna One only being a temporary group) and the former slowly dwindling in popularity unfortunately as well. Other notable introductions are ITZY who debuted in 2019 and then BLACKPINK who have very little activity in the first three years, but then did very well in 2020.
...
Interestingly, we see an increase then sharp decrease over the years in songs rewarded crowns.
...
-   While pretty much 50/50 on gender wins, there are certain music shows that seem biased towards specific genders (MCountdown and Inkigayo female, the rest male)
- All but Inkigayo had a semblance of a normal points distribution
-   Crown achievements have gone down over the years, perhaps due to the diversification of the industry (more groups debuting at a higher rate than before)
