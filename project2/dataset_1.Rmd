...
This data has one row per student and includes the following columns:
...
Ideally, we flatten this table to be one row per student, test, and term number rather than having columns for term scores. Additionally, the `sex and age` column should be split into two columns. Lastly, I would prefer if the `test number` column was just an int value rather than a chr with the word `test` in front of each number, and I'll do the same to term as it'll also end up being like `term number` at first.
...
```{r pivot}
student_data_pivoted <- pivot_longer(student_data, cols=6:8, names_to="term_number", values_to="test_score")
...
Next, we'll be removing the `test ` and `term.` words from the test and term number columns and then converting them to numbers. We'll also rename the column `test.number` to `test_number` for consistency.

```{r clean_test_term}
student_data_pivoted$test.number <- gsub("test ", "", student_data_pivoted$test.number)
student_data_pivoted$test.number <- as.numeric(as.character(student_data_pivoted$test.number))

student_data_pivoted$term_number <- gsub("term\\.", "", student_data_pivoted$term_number)
student_data_pivoted$term_number <- as.numeric(as.character(student_data_pivoted$term_number))
...
```{r split_sex_age}
student_data_pivoted <- student_data_pivoted |> separate_wider_delim(sex.and.age, delim="_", names = c("sex", "age"))
...
On average, students performed best in term 2, however the median test score in term 3 was higher. There was more variance in term 3 as the range is from 63 to 94 while the other terms were a little closer in score, signalling that there was a lot more variance in term 3.

Now, does sex affect how well students do?
